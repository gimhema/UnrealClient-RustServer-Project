...
impl GameLogicMain {
    ...
    fn try_send_udp(&self, addr: SocketAddr, payload: Vec<u8>) {
        if let Some(tx) = &self.net_tx {
            if let Err(_) = tx.send_udp(addr, payload) {
                ...
                eprintln!("[game_logic] UDP queue is full; dropping packet to {}", addr);
            }
        } else {
            eprintln!("[game_logic] NetSender not set; cannot send UDP");
        }
    }
    ...
    pub fn broadcast_msg_udp_all(&self, data: Vec<u8>) -> usize {
        if let Some(tx) = &self.net_tx {
            ...
            eprintln!("[game_logic] NetSender not set; cannot broadcast UDP");
            0
        }
    }
    ...
    pub fn send_msg_udp_to_entity(&self, entity_id: u32, payload: Vec<u8>) -> bool {
        if let Some(tx) = &self.net_tx {
            ...
            eprintln!("[game_logic] NetSender not set; cannot send UDP to entity {}", entity_id);
            false
        }
    }
}
