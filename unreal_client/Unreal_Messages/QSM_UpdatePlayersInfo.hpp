...
private:
    static bool try_deserialize_at(const std::vector<uint8_t>& buf, size_t start, UpdatePlayersInfo& out) {
        size_t off = start;
        ...
        uint32_t __mid = 0;
        uint32_t __pid = 0;
        std::string __PlayerName;

        if (!get_u32(__mid)) return false;
        if (!get_u32(__pid)) return false;
        if (!get_str(__PlayerName)) return false;

        out.mid = __mid;
        out.pid = __pid;
        out.PlayerName = std::move(__PlayerName);
        return true;
    }

public:
    static UpdatePlayersInfo deserialize(const std::vector<uint8_t>& buffer) {
        UpdatePlayersInfo out;
        size_t off0 = 0;
        if (buffer.size() >= 8) {
